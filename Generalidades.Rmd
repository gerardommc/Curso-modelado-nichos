...
```{r patron, echo = F, fig.cap="Patrones de puntos de densidad variable. A la izquierda células de mucosa gástrica en corte histológico. A la derecha, cúmulos de galaxias (Baddeley et al. 2016)."}
...
```{r tipos-puntos, echo = F, fig.cap="Ubicaciones de dos especie de árbol, abeto y roble, en la misma parcela."}
...
```{r tipos-puntos-2, echo=F, fig.cap="Ubicaciones de árboles con mediciones de diámetro.", fig.dim="30%"}
...
```{r dimensiones, echo = F, fig.cap="Ejemplos de procesos de puntos en 1 y 3 dimensiones"}
...
```{r covar, echo = F, fig.cap="Datos de Beilschmiedia pendula sobre un modelo digital de elevación."}
...
```{r cont-estim, echo = F, fig.cap="Se analiza un patrón para predecir variación contínua."}
...
```{r colecciones, echo = F, fig.cap="Repositorios comunes de información de biodiversidad."}
...
### Diferencias con regresión simple
...
### Relación con métodos populares
...
### Ventajas sobre otros métodos de análisis
