...
covar <- cov(r.df[, 3:5])
md <- mahalanobis(r.df[, 3:5], center = centroide$mean, 
                  cov = covar)
head(md)
```
...
```{r echo=T, fig.height=3.5, fig.width=3.5, fig.align='center'}
...
puntos.v <- vect(data.frame(lon = puntos.2$x, lat = puntos.2$y)[1:15, ])
